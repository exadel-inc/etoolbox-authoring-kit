...
public class DependsOnHandler implements Handler, BiConsumer<Element, MemberWrapper> {
    ...
    public void accept(Element element, MemberWrapper memberWrapper) {
        ...
        handleDependsOnRefValue(element, memberWrapper);
    }
     ...
     * @param memberWrapper Current {@code MemberWrapper} instance
     */
    private void handleDependsOnRefValue(Element element, MemberWrapper memberWrapper) {
        DependsOnRef value = PluginReflectionUtility.getMemberAnnotation(memberWrapper.getMember(), DependsOnRef.class);
        if (value == null) {
            return;
        }
        ...
        if (StringUtils.isBlank(dependsOnRefName)) {
            dependsOnRefName = memberWrapper.getMember().getName();
        }
