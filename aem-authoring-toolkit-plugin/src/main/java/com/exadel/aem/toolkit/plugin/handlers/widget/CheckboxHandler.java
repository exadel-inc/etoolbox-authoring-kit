...
class CheckboxHandler implements BiConsumer<Source, Target> {
    ...
    public void accept(Source source, Target target) {
        ...
        Predicate<Method> mappingFilter = PluginObjectUtility.getPropertyMappingFilter(checkbox);
        ...
        for (Class<?> sublistClass : checkbox.sublist()) {
            ...
            for (Source innerSource : sources) {
                ...
                Target checkboxElement = target.getOrCreateTarget(innerSource.getName())
                        .attribute(DialogConstants.PN_SLING_RESOURCE_TYPE, ResourceTypes.CHECKBOX)
                        .attributes(innerCheckbox, PluginObjectUtility.getPropertyMappingFilter(innerCheckbox));
