...
#### Use of WriteMode. Merging the manual and auto-generated XML
...
If the component directory exists, the plugin cleans it up before storing autogenerated content. The cleanup is performed separately for the possible _cq:dialog_, _cq:EditConfig_, _cq:childEditConfig_, etc. nodes. E.g., if there is a *_cq_dialog.xml* file already in the directory, and the component has a `@Dialog` annotation, the previous XML file is deleted. But if there is already a *_cq_EditConfig.xml* file and the dialog does NOT have an `@EditConfig` annotation, the `_cq_EditConfig.xml` file will persist.

There is also a way to merge the pre-existent XML and the autogenerated one. Use `writeMode = writeMode.MERGE` for that. Note: in such a case, the autogenerated file is stored and then the pre-existent one is merged onto it node by node. This way, you can deliberately leave an XML file in the component folder and use it as some sort of "patch" over the auto-generated file if the latter misses some features.
