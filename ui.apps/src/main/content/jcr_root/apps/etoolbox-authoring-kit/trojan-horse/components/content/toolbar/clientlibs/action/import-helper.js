...
(function (author, ns) {
    ...
    ns.EToolboxTrojanHorse = ns.EToolboxTrojanHorse || {}

    /** Join JCR path parts */
    ns.EToolboxTrojanHorse.joinJCRPath = function (path1, path2, ...rest) {
        if (rest.length > 0) return ns.EToolboxTrojanHorse.joinJCRPath(ns.EToolboxTrojanHorse.joinJCRPath(path1, path2), ...rest);
        return (path1 + '/' + path2).replace(/\/(\/)+/g, '/');
    };

    /** Join and normalize JCR path. Removes ending '/' */
    ns.EToolboxTrojanHorse.resolveJCRPath = function (path, ...rest) {
        if (rest.length > 0) return ns.EToolboxTrojanHorse.resolveJCRPath(ns.EToolboxTrojanHorse.joinJCRPath(path, ...rest));
        return path.replace(/\/$/, '').replace(/\/_jcr_content\//g, '/jcr:content/');
    }

    /** Resolve path to the JCR content */
    ns.EToolboxTrojanHorse.resolveJCRContentPath = function (path) {
        return path ? ns.EToolboxTrojanHorse.joinJCRPath(path, '/_jcr_content/') : '';
    };

    /** Check if the passed resource type can be placed inside of passed path */
    ns.EToolboxTrojanHorse.canInsert = function (resourceType, parentPath) {
        ...
        const resolvedPath = ns.EToolboxTrojanHorse.resolveJCRPath(parentPath);
    ...
    const findRootName = (content) => {
        if (Object.hasOwnProperty.call(content, 'trojan-horse-parsys')) return 'trojan-horse-parsys';
        return null;
    };
     ...
     * @returns result path
     * */
    ns.EToolboxTrojanHorse.setupInsertionFormTarget = function ($el, path) {
        path = ns.EToolboxTrojanHorse.joinJCRPath(path, '/');
        $el.closest('form[action]').attr('action', path);
        return path;
    };

    /** Fetch action successful response handler to resolve first content component */
    ns.EToolboxTrojanHorse.mapImportResourcePath = function (response, parentName, parentPath) {
        try {
            ...
            for (const name of nodeNames) {
                ...
                const path = ns.EToolboxTrojanHorse.resolveJCRPath(parentPath, '/', rootName, '/', name);
    ...
    ns.EToolboxTrojanHorse.mapImportResourcePathError = () => '';
